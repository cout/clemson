...
// A side effect of this operation is that l ends up in sorted order.
Biname& Biname::smerg(Biname_List & l) {
    l.sort();
    
    i = 1;                                      // the intial 1

    Biname_List::iterator it;
    for(it = l.begin(); it != l.end(); ++it) {
        *this += *it;                           // concatenate each element
    }
