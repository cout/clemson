...
int Spantree::next_tree() {
	...
	while(j < graph.num_nodes) {
		...
		// Check to see if there are enough edges left over to build a tree.
		// If not, then set backtrack.  We set backtrack if:
		//   (num edges in spanning tree) - (index of the edge) + 1 is less
		//   than (number of nodes in graph) - (stage of building the subtree).
		if((graph.num_edges - i - 0) < (graph.num_nodes - j))
			backtrack = true;

		// We must backtrack
		if(backtrack) {
			j--;                    // We are removing elements
			if(j == 0) return j;    // If no more elements, then no more trees
			i = f[j-1] + 1;
            f.resize(j-1);          // This should remove elements from f
			backtrack = false;      // We are no longer backtracking
		...
		// Determine which edge to visit next
			...
			if(x == y) {
				...
				// stack (so we can rollback to a previous state if we decide
				// to backtrack).  Initially, g[0][v] is equal to v.  As we add
				// an edge, we update the component numberings so we know which
...
// gbuild produces the first nxn component status array for a given edge
// list and edge-index selector.
void Spantree::gbuild() {
	int v, j;

    g.resize(graph.num_nodes);
	for(int j = 0; j < graph.num_nodes; j++) {
		g[j].resize(graph.num_nodes);
