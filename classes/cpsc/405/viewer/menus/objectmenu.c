...
static void save_file(GtkWidget *w, GtkFileSelection *selector) {
	gchar *filename;
	struct stat statbuf;
	...
	if(stat(filename, &statbuf)==0) {
		if(!message_yesno("Are you sure you want to overwrite "
			"this file?")) return;
	}
	save_aw(filename, &aw_object[curr_obj]);
}
...
void on_create_curved_model_activate(GtkMenuItem *m, gpointer data) {
	create_gl2_window();
}
