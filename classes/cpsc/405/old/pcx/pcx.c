	...
	for (j = 0; j < image_height; j++) {
		for (i = 0; i < image_width; i++) {
			...
			if(PCX_TEXRES == GL_RGB) {
				buffer[3*(j * image_width + i) + 0] =
					pal.r;
				buffer[3*(j * image_width + i) + 1] =
					pal.g;
				buffer[3*(j * image_width + i) + 2] =
					pal.b;
			} else if(PCX_TEXRES == GL_RGB16) {
				buffer[2*(j * image_width + i)] =
					(pal.r >> 2) << 10 |
					(pal.g >> 2) << 5 |
					(pal.b >> 3);
			}
...
int bind_pcx(const char *filename, int image_width, int image_height, int texnum) {
	...
	texture = malloc(3 * image_width * image_height);

	if(load_pcx(texture, filename, &file_width, &file_height) == PCX_OK) {
