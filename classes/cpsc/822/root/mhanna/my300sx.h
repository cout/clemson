/*
template .h file for glint300sx driver
*/
...
#define MAX_DMA (128*1024 - ONE_PAGE)
#define V_OFF 0
...
#define AreaStippleMode (0x81a0)
 ...
 ... etc. ... add those needed


#define EVAL_K_CONTROL(register) (*(glint_board.k_control_base+(register>>2)))

#define LOAD_K_CONTROL(register,value) 
	((*(glint_board.k_control_base+(register>>2)))=value)

#define LOAD_DAC(register,value) 
	((*(glint_board.k_dac_base+(register>>2)))=value) 

#define LOAD_K_FRAMEB(pixel,value) 
	((*(glint_board.k_frame_buffer_base+(pixel)))=value)

#define LOAD_K_LOCALB(pixel,value) 
	((*(glint_board.k_local_buffer_base+(pixel)))=value)

#define FIFOSYNC while(EVAL_K_CONTROL(InFIFOSpace)<16)

struct glint_device {

	... store all the base addresses here ....
