...
/* Lab 6 -- global variables, oh well */
static double sum_of_services = 0.0;
static double sum_of_waits = 0.0;
static double num_requests = 0.0;
double get_ide_request_data(int n) {
	switch(n) {
		case 0:
			sum_of_services = 0.0;
			sum_of_waits = 0.0;
			num_requests = 0.0;
			return 0;
		case 1: return sum_of_services; break;
		case 2: return sum_of_waits; break;
		case 3: return num_requests; break;
	}
	return 0.0;
}
...
void ide_end_request(byte uptodate, ide_hwgroup_t *hwgroup)
{
	...
	num_requests++;
	sum_of_services += 
		(tv.tv_usec - rq->start_of_service.tv_usec) / 1000 +
		(tv.tv_sec - rq->start_of_service.tv_sec) * 1000;
	sum_of_waits += (rq->
		 start_of_service.tv_usec - rq->start_of_wait.tv_usec) / 1000 +
		(rq->start_of_service.tv_sec - rq->start_of_wait.tv_sec) * 1000;
	*/
	get_ptimer(&end_time);
	sum_of_services += calc_ptimer(rq->start_of_service, end_time);
	sum_of_waits += calc_ptimer(rq->start_of_wait, rq->start_of_service);
