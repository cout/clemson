...
#define TAG_MASK 0xFFFFFFF0
#define WORD_MASK 0x00000003
#define SET_MASK 0x0000000C
#define SET_SHIFT 2
...
unsigned long cache[NUM_SETS][NUM_CACHE_LINES][NUM_WORDS_PER_LINE];
unsigned long int tags[NUM_SETS][NUM_CACHE_LINES];
unsigned long int mem_count[NUM_SETS][NUM_CACHE_LINES];
unsigned long int valid_bits[NUM_SETS][NUM_CACHE_LINES];
unsigned long int cache_hits;
unsigned long int cache_misses;

#define IS_VALID(x,y) (valid_bits[(x)][(y)])
...
*/
void LRU_Access(unsigned long int set, unsigned long int cache_line){
	int i;
	for(i = 0; i < NUM_CACHE_LINES; i++){
		mem_count[set][i]++;
	}	
	valid_bits[set][cache_line] = 1;
	mem_count[set][cache_line] = 0;
}
...
unsigned long int Read_Cache(unsigned long int address){
	int i, j, invalid_line, victim;
	unsigned long int word;
	unsigned long int tag;
	unsigned long int set;
	unsigned long int set_something;
	unsigned long int cache_line;

	tag = address & TAG_MASK;
	word = address & WORD_MASK;
	set_something = address & SET_MASK;
	set = set_something >> SET_SHIFT;
...
//		printf("Cache miss\n");
                if(invalid_line != -1){
                        for(j = 0; j < NUM_WORDS_PER_LINE; j++){
                                cache[set][invalid_line][j] = rammem[tag + set_something + j];
                        }
                        tags[set][invalid_line] = tag;
                        LRU_Access(set, invalid_line);
                        return cache[set][invalid_line][word];
                }
                else{
                        for(j = 0; j < NUM_CACHE_LINES; j++){
                                if(mem_count[set][j] > mem_count[set][victim]){
                                        victim = j;
                                }
                        }
                        for(j = 0; j < NUM_WORDS_PER_LINE; j++){
                                cache[set][victim][j] = rammem[tag + set_something + j];
                        }
                        tags[set][victim] = tag;
                        LRU_Access(set, victim);
                        return cache[set][victim][word];
                }
        }
        else{
	...
	set = set_something >> SET_SHIFT;
        ...
        if(i == NUM_CACHE_LINES){
                ...
                if(invalid_line != -1){
                        ...
                        tags[set][invalid_line] = tag;
                        LRU_Access(set, invalid_line);
                        rammem[tag + set_something + word] = data;
                        cache[set][invalid_line][word] = data;
                }
                else{
                        for(j = 0; j < NUM_CACHE_LINES; j++){
                                if(mem_count[set][j] > mem_count[set][victim]){
