...
/*
** Variables needed for an associative cache
*/
...
typedef unsigned long int cache_line[NUM_WORDS_PER_LINE];
cache_line cache[NUM_CACHE_LINES];
...
void Init_Cache(){
	...
	for(j = 0; j < NUM_CACHE_LINES; j++) {
		memset(cache[j],0,sizeof(cache_line));
		tags[j] = 0;
		mem_count[j] = 0;
	}
	valid_bits = cache_hits = cache_misses = 0;
}

/*
** Access - this function just handles the bookkeeping for an
** LRU replacement scheme.
*/
void LRU_Access(unsigned long int cache_line){
	int j;
	valid_bits |= (1 << cache_line);
	for(j = 0; j < NUM_CACHE_LINES; j++) mem_count[j]++;
...
unsigned long int Read_Cache(unsigned long int address){
    int j, line = -1;
    unsigned long int block, word;

    block = address & TAG_MASK;  /* isolates the correct memory block */
    word = address & WORD_MASK;  /* isolates the word within the block */
    ...
    for(j = 0; j < NUM_CACHE_LINES; j++) {
	    ...
	    if(block == tags[j]) break;
    }
//    if(block != tags[j]) {
    if(j == NUM_CACHE_LINES){
	cache_misses++;
	if(line != -1) {
            /* the requested value isn't in the cache, but we have an empty
               cache line to put it in */
	    for(j = 0; j < NUM_WORDS_PER_LINE; j++)
                cache[line][j] = rammem[block + j];
	    tags[line] = block;
            LRU_Access(line);
	    ...
	    int biggest = 0;
            /* there are no empty cache lines, so some line must be evicted */
            for(j = 1; j < NUM_CACHE_LINES; j++) {
		if(mem_count[biggest] < mem_count[j])
		    biggest = j;
            }
	    for(j = 0; j < NUM_WORDS_PER_LINE; j++) {
                cache[biggest][j] = rammem[block + j];
            }   
	    tags[biggest] = block;
            LRU_Access(line);
            return cache[biggest][word];
        }
    }
...
/*	unsigned long instr = rammem[NPC];*/
	unsigned long instr = Read_Cache(NPC);
	unsigned long rsrc1 = (instr >> 14) & 0x0000001f;
	unsigned long rsrc2 = instr & 0x0000001f;
...
/*
** decode - this function decodes a fetched instruction in the
