...
unsigned long int Read_Cache(unsigned long int address){
	...
	int invalid_line = -1;
	int victim = 0;
	unsigned long int tag, word, set, cache_line;
	...
	if((IS_VALID(cache_line)) && (tags[cache_line] == tag)) {
		cache_hits++;
		LRU_Access(cache_line);
		return cache[cache_line][word];
	} else {
		cache_misses++;
		for(j = 0; j < NUM_WORDS_PER_LINE; j++)
			cache[cache_line][j] = rammem[tag + set + j];
		tags[cache_line] = tag;
		LRU_Access(cache_line);
		return cache[cache_line][word];
	}
...
void Write_Cache(unsigned long int address, unsigned long int data){
	...
	if(IS_VALID(cache_line) && tags[cache_line] == tag) {
		cache_hits++;
		LRU_Access(cache_line);
		cache[cache_line][word] = data;
		rammem[tag + set + word] = data;
	} else {
		cache_misses++;
		for(j = 0; j < NUM_WORDS_PER_LINE; j++)
			cache[cache_line][j] = rammem[tag + set + j];
...
char **argv;
{
	...
	if (argc < 2) {
      printf("ERROR:  %s sparc.prg\n", argv[0]);
		...
		NPC = PC = 0;
		Init_Cache(); 
		registers[30] = 0x00000400;  /* sets frame pointer */
